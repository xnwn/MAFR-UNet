...
class DiceLoss(nn.Module):
    ...
    def to_one_hot(self, tensor):
        ...
        for i in range(self.num_class):
            tensor_temp = tensor == i
            tensor_list.append(tensor_temp.unsqueeze(1))  # 24*224*224 -> 24*1*224*224
        return torch.cat(tensor_list, dim=1).float()

    @staticmethod
    def dice_loss(predict, label):
        intersect = torch.sum(predict * label.float())
        smooth = 1e-5

        y_sum = torch.sum(label.float() * label.float())
        z_sum = torch.sum(predict * predict)
        return 1 - ((2 * intersect + smooth) / (z_sum + y_sum + smooth))

    def forward(self, predict, label, softmax, weight=None):
        if softmax:
            predict = torch.softmax(predict, dim=1)
        if weight is None:
            weight = [1] * self.num_class

        label = self.to_one_hot(label)
        assert predict.size() == label.size(), \
            f"predict shape {predict.size()} & target shape {label.size()} don't match"

        loss = 0.
        for i in range(self.num_class):
            dice_loss = self.dice_loss(predict[:, i], label[:, i])
