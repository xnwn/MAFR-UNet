...
if __name__ == "__main__":
    '''Convert all nrrd files to nii files
        ...
        KiTS_origin K1 ~ K20 || Dongyang D1 ~ D18 || Rider R1 ~ R18

    '''
    convert_files("KiTS_origin", "K", 20) # # kits_origin is the folder where KiTS_origin.zip is stored after decompression

    '''KiTS_origin preprocess
        Slice thickness(mm): x * x * 0.5 ~ y * y * 5
        To standardize the data, we resampled it to 1 * 1 * 1
    
        Pixel value: K7 ranges from -1024 to 4054, K18 ranges from -2048 to 5064, the rest range from 0 to 2505 ~ 4095
        The HU value range of common CT images: -1024 ~ 3071
        The pixel value range of common CT images: 0 ~ 4095
        Based on the above range, we believe that most of the data is stored in pixel values
        After analyzing each sample one by one, it was found that only K18 stored the HU value
        To standardize the data, We convert the pixel values of the remaining samples to the more commonly used HU values
        Referring to the DICOM conversion formula, Hu = pixel * slope + intercept, assume slope = 1 and intercept = -1024
        ...
        After resampling, the size of the image changes and can be uniformly restored to the initial size of 512 * 512
        ...
        The code we use is as follows:
            ...
            idx_list = random.sample(range(1, 20), 6)
            print(idx_list)  # [7, 13, 9, 18, 10, 12]
    '''
    file_process("KiTS_origin", "K", 20, False, [17], 1, -1024)
    nii2npy_npz("KiTS_origin", "KiTS", "K", 20, [7, 13, 9, 18, 10, 12])
