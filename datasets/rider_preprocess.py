...
if __name__ == "__main__":
    ...
    '''
    # Rider_origin is the folder where Rider_origin.zip is stored after decompression
    # convert_files("Rider_origin", "R", 18)

    '''Rider preprocess
        Slice thickness(mm): x * x * 0.625 ~ y * y * 2.5
        To standardize the data, we resampled it to 1 * 1 * 1
    
        Pixel value: R4 ranges from -2000 to 2679, D18 ranges from -2000 to 2880,
                     the rest range from -2000 to 4095
                     The target pixel values of R14-R16 and R18 differ from those of the other samples by about 200.
                     Since the difference is not large, 
                     it is not treated separately and R18 is only used as a test set in the future.
                     
        The HU value range of common CT images: -1024 ~ 3071
        The pixel value range of common CT images: 0 ~ 4095
        Based on the above range, we believe that this dataset has the same problem as KiTS
        So we do the same operation as KiTS for all samples 
        ...
        According to the above analysis, R18 is used as the test set and R14-R16 is used as the training set.
        The code we use is as follows:
            import random
            
            idx_list = random.sample(range(1, 18), 6)
            print(idx_list)  # [4, 5, 9, 7, 13, 12]
    '''
    # file_process("Rider_origin", "R", 18, False, [], 1, -1024)
    nii2npy_npz("Rider_origin", "./Rider", "R", 18, [4, 5, 9, 7, 13, 12])
